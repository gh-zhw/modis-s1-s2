...
generator = torch.load(r"D:\Code\modis-s1-s2\model\wgan-1\wgan_generator_epoch_300.pth")
generator = generator.to(device)
generator.eval()

writer = SummaryWriter(r"D:\Code\modis-s1-s2\logs\test")

test_data_len = len(test_dataloader)
...
for mini_batch in test_dataloader:
    ...
    metric["mae"] += (mae / test_data_len)
    metric["mse"] += (mse / test_data_len)
    metric["sam"] += (sam / test_data_len)
    metric["psnr"] += (psnr / test_data_len)
    metric["ssim"] += (ssim_value / test_data_len)
