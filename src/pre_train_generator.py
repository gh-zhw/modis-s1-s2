...
from utils import generated_S2_to_rgb, L1_Loss_for_bands, L2_Loss_for_bands
...
train_loss = {"L_loss": [], "L_loss_band_1": [], "L_loss_band_2": [],
              "L_loss_band_3": [], "L_loss_band_4": [], "L_loss_band_5": [], "L_loss_band_6": [],
              "L_loss_band_7": [], "L_loss_band_8": []}
...
for epoch in range(epochs):
    ...
    for mini_batch in train_dataloader:
        ...
        if step % 20 == 0:
            end_time = time.time()
            print("[step {}/{}] train_loss = {}  {}s".format(
                step, total_step, L_loss.item(), round(end_time - start_time, 2)))
...
torch.save(generator, f"D:\Code\MODIS_S1_S2\model\pre_train_generator_epoch_{epochs}.pth")
